...
public class DataBaseRenderer implements Renderer {
	...
	@Autowired
	public DataBaseRenderer(JdbcTemplate jdbcTemplate, DataProvider<Product> productProvider,
			DataProvider<Category> categoryProvider, Request<Category> categoryRequest) {
	...
	public void insertProduct(Product product) {
		String sql = "INSERT INTO PRODUCTS (product_id, name, description, category_id, price, stock_quantity," +
				" image_url, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
		jdbcTemplate.update(sql, product.product_id, product.name, product.description, product.category_id,
				product.price, product.stock_quantity, product.image_url, product.created_at, product.updated_at);
	}
}

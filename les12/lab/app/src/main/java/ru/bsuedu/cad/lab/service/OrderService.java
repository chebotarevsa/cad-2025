...
public class OrderService {
	...
	public Order createOrder(int customerId, int productId, int quantity) {
		Customer customer = customerRepository.findById(customerId)
				.orElseThrow(() -> new RuntimeException("Client not found with ID: " + customerId)); //Не найден клиент с ID:

		Product product = productRepository.findById(productId)
				.orElseThrow(() -> new RuntimeException("Product ID not found: " + productId));//Не найден продукт с ID:
		...
		logger.info("Order ID created: {}", savedOrder.getOrderId());//Создан заказ ID

		return savedOrder;
	}

	@Transactional //получение всех заказов
